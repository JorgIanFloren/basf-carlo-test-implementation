				...
				<xsl:for-each select="FMSDOM_GOODS/FMSDOM_GOOD">
					...
					<SEG TYPE="RFF">
						<ELEMENT>
							<FIELD>BM</FIELD>
							<FIELD>
								<xsl:value-of select="../../FMS_BL/BL_NBR"/>
							</FIELD>
						</ELEMENT>
					</SEG>
					<SEG TYPE="DTM">
						<ELEMENT>
							<FIELD>95</FIELD>
							<FIELD>
								<xsl:value-of select="replace(../../FMS_BL/DATE_OF_ISSUE, '-', '')"/>
							</FIELD>
							<FIELD>102</FIELD>
						</ELEMENT>
					</SEG>
					
					<xsl:variable name="linkedPRETR">
						<xsl:for-each select="FMSDOM_GOOD_GIETS/FMSDOM_GOOD_GIET">
							<xsl:variable name="GIET" select="@RELATION_IDENTIFIER"/>
							<xsl:for-each select="../../../../FMSDOM_TRANSPORTS/FMSDOM_TRANSPORT[TRANSPORT_MODE = 'PRE' and FMSDOM_TRANSPORT_GIETS/FMSDOM_TRANSPORT_GIET/@RELATION_IDENTIFIER = $GIET]">
								<xsl:copy-of select="."/>							
							</xsl:for-each>
						</xsl:for-each>
					</xsl:variable>
					<xsl:for-each-group select="$linkedPRETR/FMSDOM_TRANSPORT" group-by="@KEY">
						<SEG TYPE="DTM">
							<ELEMENT>
								<FIELD>334</FIELD>
								<FIELD>
									<!--2021-05-06T00:00:00-->
									<xsl:value-of select="replace(tokenize(FMSDOM_TRANSPORT_STOPS/FMSDOM_TRANSPORT_STOP[HANDLING_TYPE = 'UNLOAD' or  HANDLING_TYPE = 'DROPOFF']/ACTUAL_TIME/text(), 'T')[1], '-', '')"/>
								</FIELD>
								<FIELD>102</FIELD>
							</ELEMENT>
						</SEG>
						<SEG TYPE="DTM">
							<ELEMENT>
								...
								<FIELD>
									<!--2021-05-06T00:00:00-->
									<xsl:value-of select="replace(tokenize(FMSDOM_TRANSPORT_STOPS/FMSDOM_TRANSPORT_STOP[HANDLING_TYPE = 'UNLOAD' or  HANDLING_TYPE = 'DROPOFF']/ACTUAL_TIME/text(), 'T')[2], ':', '')"/>
								</FIELD>
								<FIELD>402</FIELD>
							</ELEMENT>
						</SEG>
					...
					<xsl:for-each-group select="$linkedTR/FMSDOM_TRANSPORT" group-by="@KEY">	
						...
						<SEG TYPE="DTM">
							<ELEMENT>
								<FIELD>133</FIELD>
								<FIELD>
									<xsl:value-of select="replace(tokenize(FMSDOM_TRANSPORT_STOPS/FMSDOM_TRANSPORT_STOP[PLACE_QUALIFIER = '9']/START_TIME, 'T')[1], '-', '')"/>
								</FIELD>
								<FIELD>102</FIELD>
							</ELEMENT>
						</SEG>
