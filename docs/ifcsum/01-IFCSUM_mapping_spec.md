...
| Transform library | `src/main/dw/IfcsumModule.dwl` |
| Entry point | `src/test/dw/BasfIfcsum.dwl` |
...
`RFF+LI` is the key `00-basf.md` names, and it is exactly what `IftminModule` wrote onto the
...
`_<SEGMENT>` key suffix (`CommonModule`), not by position key; see
